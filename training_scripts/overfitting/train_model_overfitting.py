...
def train_model(model_name, get_model, overfitting_path):
    ...
    print(f"Training {model_name} on limited data (overfitting experiment) on: {device}\n")
    ...
    # 4. INIT MODEL
    model = get_model(device, version="simple")
    ...
    ensure_parent_dir(overfitting_path)
    torch.save(model.state_dict(), overfitting_path)
    print(f"Best model saved to {overfitting_path}")
    ...
    fig.tight_layout()
    plt.savefig('training_curves_{model_name}_overfitting.png', dpi=150, bbox_inches='tight')
    print("Training curves saved to training_curves_{model_name}_overfitting.png")
    plt.show()

    print("\n{model_name} overfitting experiment complete!")

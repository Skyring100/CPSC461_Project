...
from common import (
    ...
    prepare_dataset_root, BATCH_SIZE, CNN_EXPERIMENT_TEMPLATE, format_experiment_path
)

# 1. SETUP
device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
print(f"Training CNN experiments on: {device}")
...
for experiment_num, percentage in enumerate(PERCENTAGES, 1):
    ...
    # Save model
    save_path = format_experiment_path(CNN_EXPERIMENT_TEMPLATE, percentage)
    torch.save(model.state_dict(), save_path)
    print(f"Model saved to {save_path}")

print(f"\n{'='*60}")
print("All CNN experiments completed!")
print(f"{'='*60}")
